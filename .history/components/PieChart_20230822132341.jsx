...
const PieChart = () => {
  ...
  const total = spendingData.reduce((sum, sd) => sum + sd.value, 0);
    ...
    <View style={{right: 27}}>
      <Svg width={width} height={height}>
        <G transform={`translate(${cx}, ${cy})`}>
          {spendingData.map((sd, index) => {
            endAngle = startAngle + (sd.value / total) * 2 * Math.PI;
          ...
          <Circle cy="0" cx="0" r={innerRadius} fill="blue" />
        </G>
      </Svg>
    </View>
  );
