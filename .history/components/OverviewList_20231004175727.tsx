...
const OverviewList = () => {
   ...
   const findOverview = async () => {
     ...
     if (result !== null) setOverview(JSON.parse(result));
   }
   ...
   const handleOnSubmit = async (title, goalAmount, goalProgress) => {
     ...
     const updatedOverviews = [...overview, overview];
      ...
      />
        {!overview.length ? (
            <View style={[StyleSheet.absoluteFillObject, styles.addNotesContainer]}>
              <Text style={styles.addNotes}>Add credit score - Emergency fund - Net worth info</Text>
            </View>
          ) : null}
          <TouchableOpacity
            style={styles.fab}
